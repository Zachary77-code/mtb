...
负责基于病历原文进行病理学和影像学分析，使用 PubMed 和 GDC 工具查询相关数据。
"""
...
class PathologistAgent(BaseAgent):
    ...
    - 评估 IHC 标记物的意义
    - 使用 PubMed 查询病理相关文献
    - 使用 GDC 查询组织学类型的突变频率（TCGA/ICGC 数据）
    """

    def __init__(self):
        tools = [
            PubMedTool(),      # 文献检索（病理相关）
            GDCTool(),         # 组织学类型突变频率（TCGA/ICGC 数据）
        ]
        super().__init__(
            role="Pathologist",
...
3. 使用 search_gdc 查询该组织学类型在相应癌种中的突变频率分布（TCGA/ICGC 数据）
4. 分析病理学发现的临床意义
